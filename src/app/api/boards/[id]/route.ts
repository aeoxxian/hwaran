...
function mapNotionPageToPost(page: Record<string, unknown>): BoardPost {
    ...
    return {
        ...
        category,
        status: (getTextProperty(page, "상태") ||
            "대기") as BoardPost["status"],
        ...
        approvalStatus:
            (getTextProperty(
                page,
                "승인상태",
            ) as BoardPost["approvalStatus"]) || "approved",
        visibility:
            (getTextProperty(page, "공개범위") as BoardPost["visibility"]) ||
            "public",
    };
}
...
export async function GET(
    ...
    try {
        ...
        return NextResponse.json({
            post: mapNotionPageToPost(page as Record<string, unknown>),
        });
    } catch {
        return NextResponse.json(
            { error: "게시글을 찾을 수 없습니다." },
...
export async function PATCH(
    ...
    try {
        const page = (await notion.pages.retrieve({ page_id: id })) as Record<
            string,
            unknown
        >;
        ...
        if (typeof body.title === "string")
            properties["제목"] = { title: [{ text: { content: body.title } }] };
        if (typeof body.content === "string") {
            const contentProp =
                post.category === "lost-found" ? "설명" : "내용";
